...
AGENT_PROMPTS: dict[str, dict[str, Any]] = {
    "business_analyst": {
        ...
        "description": (
            ...
            "(performance, privacy, accessibility) where relevant. "
            "Keep the document proportionate to the size of the idea: a small feature "
            "warrants a short document — do not pad sections with boilerplate that does "
            "not apply to this specific idea. {date_context}"
        ),
        "expected_output": (
    ...
    "security_auditor": {
        ...
        "description": (
            ...
            "Do not take the developer's stated security measures at face value: read the "
            "code line by line and hunt for concrete defects, including dead or pointless "
            "code, type-annotation mismatches with runtime behaviour, validation that "
            "checks one value but stores another, and any discrepancy between what a "
            "comment or docstring claims and what the code actually does. Report each "
            "such defect as a finding even when it is not directly exploitable. "
